...
await page.evaluate((t) =&gt; {
  ...
  },
  {
    ...
    layman: {
      problem:
        'A class of web apps draws part or all of their UI inside a <code>&lt;canvas&gt;</code> element &mdash; the browser sees one big rectangular pixel buffer, and everything inside (text, buttons, input boxes) is just paint. Most of the apps automation actually targets &mdash; legacy enterprise systems, custom in-house tools, AS/400-to-web emulators, signature pads, custom datepickers, charting widgets with embedded interaction &mdash; ship no accessibility tree for the canvas at all. Against those, Playwright has nothing to query: no <code>&lt;label&gt;</code>, no <code>&lt;input&gt;</code>, no text node. And the cost is binary: one canvas widget anywhere in the flow stops the whole automation, because selectors cannot skip past a step they cannot interact with.',
      workaround:
    ...
    playwright: {
      kind: 'impossible',
      label: 'Structurally unreachable without an accessibility tree',
      notes: `
        <p><strong>Verdict: structurally unreachable for Playwright when the canvas surface does not expose an accessibility tree &mdash; which is the default for legacy and enterprise apps, the apps automation actually targets.</strong></p>
        <p class="mt-2">Playwright's locator engine reads the DOM. Against a canvas-only surface there is nothing to read:</p>
        <ul class="mt-2 list-disc space-y-1 pl-5">
          <li><code>page.getByLabel('Email')</code> &mdash; no <code>&lt;label&gt;</code> element exists.</li>
          <li><code>page.getByRole('textbox')</code> &mdash; no <code>&lt;input&gt;</code> element exists.</li>
          <li><code>page.getByText('Sign in')</code> &mdash; the text "Sign in" is painted pixels, not a text node.</li>
        </ul>
        <p class="mt-2"><strong>The blocking property is what makes this 5/5 rather than a softer 'workaround' number.</strong> A canvas-rendered surface anywhere in a workflow blocks the entire automation &mdash; selectors cannot skip past a step they cannot interact with. One signature-pad widget, one canvas-based chart that gates "continue", one canvas-rendered datepicker, and the whole test breaks. Effort per-canvas does not compose down to a small total when any single canvas means the run cannot complete.</p>
        <p class="mt-2">The narrow exception: a handful of <em>consumer-SaaS</em> canvas apps ship a parallel accessibility-tree HTML layer for screen readers, which Playwright's <code>getByRole</code> / <code>page.accessibility.snapshot()</code> can reach. <a href="https://www.figma.com/blog/a-step-forward-in-our-accessibility-efforts/" class="text-sky-700 underline hover:text-sky-900">Figma</a> and <a href="https://thenewstack.io/google-docs-switches-to-canvas-rendering-sidelining-the-dom/" class="text-sky-700 underline hover:text-sky-900">Google Sheets</a> are the canonical examples. But Figma is rare in the apps an enterprise automation team actually targets. The bulk of canvas-in-the-wild &mdash; Photoshop Web, Photopea, tldraw, Excalidraw, Miro, Unity/Unreal WASM games, legacy AS/400-to-web emulators, custom signature pads, in-house dashboards built on D3-canvas &mdash; ship no accessibility tree at all.</p>
        <p class="mt-2">The only theoretical Playwright path for the no-a11y case is to screenshot from Playwright, pipe to an external OCR / template-matching pipeline, and use <code>page.mouse.click(x, y)</code> at the resolved coordinates. At that point you have built a worse version of the classic AIVA &mdash; and you have moved the actual automation outside Playwright entirely.</p>
      `,
    },
    aiva: {
      passes: true,
